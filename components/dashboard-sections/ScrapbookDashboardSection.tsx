...
export default function ScrapbookDashboardSection({ section }: { section: any }) {
  return (
    <section
      className="flex flex-col md:flex-row group relative items-center py-12 md:py-20"
      style={{ isolation: "isolate" }}
    >
      {/* LEFT SIDE: Scrapbook-specific metadata */}
      <div
      ...
      {/* RIGHT SIDE: Scrapbook template preview */}
      {/* Outer: no overflow-hidden so nav arrows below the book are visible */}
      <div
        className="w-full md:w-[65%] flex-shrink-0 relative"
        style={{ height: 620, zIndex: 10 }}
      >
        {/* Inner: stops 56px above bottom — gives the nav room to sit below */}
        <div className="absolute rounded-2xl" style={{ inset: 0, bottom: 56 }}>
          <ScrapbookLayout
